...
import aguiaj.images.view.ImageWidget;
...
System.out.println("DRAG - " + e.x + " " + e.y);				
	...
	private void addPositions(List<DrawItem> items) {
		for(int row = 0; row < grid.getNumberOfRows(); row++) {
			for(int column = 0; column < grid.getNumberOfColumns(); column++) {
				aguiaj.colors.Color color = grid.getPosition(row, column).getBackground();
				// dispose
				...
				Image icon = grid.getPosition(row, column).getIcon();
				if(icon != null) {
