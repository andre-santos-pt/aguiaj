...
import aguiaj.Color;
import aguiaj.Grid;
import aguiaj.GridContract;
import aguiaj.Image;

public class GridWidgetTMp implements CanvasVisualizationWidget<Grid>  {
	...
	public void update(Grid g) {
		...
		if(g != grid) {
			grid = new GridContract(g);
			
			fontIndex = new Font(display, "Monospaced", 8, SWT.NONE);

			WIDTH = this.grid.getPositionWidth() + 1;
			if(g != null) {
		...
		if(grid != null) {
			width = (grid.getNumberOfColumns() * (WIDTH)) + 1 + BORDER;
			height = (grid.getNumberOfRows() * (WIDTH)) + 1 + BORDER;
		}
	}
...
//					Color bg = grid.getBackground(line, column);
//					Image icon = grid.getImage(line, column);
//					boolean changes =
//						icon != null && !icon.equals(icons[line][column]) ||
//						icon == null && icons[line][column] != null ||
//						!bg.equals(backgrounds[line][column]);
//
//					if(changes) {
//						Rectangle rect = new Rectangle(BORDER + (column*WIDTH)+1, BORDER + (line*WIDTH)+1, WIDTH-1, WIDTH-1);
...
//	}
	...
	public void drawObject(GC gc) {
		...
		for(int row = 0; row < grid.getNumberOfRows(); row++) {
			for(int column = 0; column < grid.getNumberOfColumns(); column++) {
				drawBackground(gc, display, row, column);								
				Image icon = grid.getImage(row, column);
				if(icon != null) {
					Common.drawImage(icon, gc, BORDER + (column * WIDTH) + 1, BORDER + (row * WIDTH) + 1, WIDTH-1, WIDTH-1, 1);					
				}
	...
	private void drawBackground(GC gc, Display display, int line, int column) {
		Color bgcolor = grid.getBackground(line, column);
		...
		gc.fillRectangle(BORDER + (column * WIDTH) + 1, BORDER + (line * WIDTH) + 1, WIDTH - 1, WIDTH - 1);
	}
	...
	private void drawLines(GC gc, Display display) {
		...
		for(int line = 0; line < grid.getNumberOfRows(); line++) {
			...
			gc.drawText(Integer.toString(line), 2, BORDER + (line * WIDTH), SWT.DRAW_TRANSPARENT);
		}
		
		for(int column = 0; column < grid.getNumberOfColumns(); column++) {
			gc.setFont(fontIndex);
			gc.setForeground(display.getSystemColor(SWT.COLOR_DARK_GRAY));
			gc.drawText(Integer.toString(column),  BORDER + (column * WIDTH)+3, 0, SWT.DRAW_TRANSPARENT);
		}
	}
